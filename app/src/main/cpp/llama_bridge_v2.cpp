...
void init_cpu_backends() {
    std::call_once(backend_once, [] {
        ...
        if (dladdr(reinterpret_cast<void *>(&init_cpu_backends), &info) && info.dli_fname) {
            ...
            const std::string dir = slash == std::string::npos ? std::string(".") : so_path.substr(0, slash);
            LOGI("Loading optimized CPU backends from %s", dir.c_str());
            ggml_backend_load_all_from_path(dir.c_str());
        } else {
            LOGI("Using default backend discovery");
...
std::string templ(const EngineHandle * handle, const std::vector<Turn> & turns, bool code_mode) {
    ...
    std::string system;
    if (code_mode) {
        system = "You are a precise offline programming assistant. Prioritize correct, runnable Python and explain assumptions briefly. Do not invent APIs. When code is requested, return complete code.";
    }
...
void strip_special_suffix(std::string & text) {
    ...
    for (const char * stop : stops) {
        const size_t len = std::strlen(stop);
        if (text.size() >= len && text.compare(text.size() - len, len, stop) == 0) { text.erase(text.size() - len); break; }
    }
}
...
extern "C" {
JNIEXPORT jlong JNICALL Java_com_musab_aragpt2_LlamaEngine_nativeLoadModel(JNIEnv * env, jobject, jstring jp, jint nc, jint nt) {
    init_cpu_backends();
...
JNIEXPORT jstring JNICALL Java_com_musab_aragpt2_LlamaEngine_nativeGenerate(JNIEnv * env, jobject, jlong hp, jstring jt, jint max_tokens, jfloat temp, jint topk, jboolean code_mode) {
    ...
    int n_tokens = -llama_tokenize(handle->vocab, prompt.c_str(), static_cast<int>(prompt.size()), nullptr, 0, first_context, true);
    if (n_tokens <= 0) { fail(env, "فشل ترميز النص"); return nullptr; }
    std::vector<llama_token> prompt_tokens(static_cast<size_t>(n_tokens));
    if (llama_tokenize(handle->vocab, prompt.c_str(), static_cast<int>(prompt.size()), prompt_tokens.data(), n_tokens, first_context, true) < 0) { fail(env, "فشل ترميز النص"); return nullptr; }

    int common_prompt_tokens = 0;
    int kv_prefix = 0;
    if (!handle->cached_prompt_tokens.empty()) {
        const size_t old_without_bos = handle->cached_prompt_tokens.size() - 1;
        const size_t n = std::min(prompt_tokens.size(), old_without_bos);
        while (common_prompt_tokens < static_cast<int>(n) && prompt_tokens[static_cast<size_t>(common_prompt_tokens)] == handle->cached_prompt_tokens[1 + static_cast<size_t>(common_prompt_tokens)]) ++common_prompt_tokens;
    ...
    const bool can_reuse = !handle->cached_prompt_tokens.empty() && common_prompt_tokens > 0 && pos_max >= kv_prefix - 1;
    if (!can_reuse) {
    ...
    std::vector<llama_token> suffix(prompt_tokens.begin() + std::min(suffix_begin, prompt_tokens.size()), prompt_tokens.end());
    if (!suffix.empty()) {
        auto batch = llama_batch_get_one(suffix.data(), suffix.size());
        if (llama_decode(handle->ctx, batch) != 0) { fail(env, "فشل تشغيل النموذج"); return nullptr; }
    }
    ...
    if (code_mode == JNI_TRUE || temp <= 0.01f) llama_sampler_chain_add(sampler, llama_sampler_init_greedy());
    else {
    ...
    while (generated < max_tokens && !handle->cancel.load()) {
        ...
        ++generated;
        if (first_ms < 0) first_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - prompt_start).count();
    ...
    LOGI("prompt=%d generated=%d first_ms=%lld tok_s=%.2f total_ms=%lld", n_tokens, generated, first_ms, tok_per_sec, static_cast<long long>(std::chrono::duration_cast<std::chrono::milliseconds>(total_end - total_start).count()));
    return env->NewStringUTF(result_pack(output, n_tokens, generated, first_ms, tok_per_sec).c_str());
}

JNIEXPORT void JNICALL Java_com_musab_aragpt2_LlamaEngine_nativeCancel(JNIEnv *, jobject, jlong hp) { auto * handle = reinterpret_cast<EngineHandle *>(hp); if (handle) handle->cancel = true; }
JNIEXPORT void JNICALL Java_com_musab_aragpt2_LlamaEngine_nativeReset(JNIEnv *, jobject, jlong hp) { auto * handle = reinterpret_cast<EngineHandle *>(hp); if (!handle || !handle->ctx) return; llama_memory_clear(llama_get_memory(handle->ctx), true); handle->cached_prompt_tokens.clear(); }
JNIEXPORT void JNICALL Java_com_musab_aragpt2_LlamaEngine_nativeFree(JNIEnv *, jobject, jlong hp) { auto * handle = reinterpret_cast<EngineHandle *>(hp); if (!handle) return; if (handle->ctx) llama_free(handle->ctx); if (handle->model) llama_model_free(handle->model); delete handle; }
}
