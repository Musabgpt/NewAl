...
public class MainActivity extends AppCompatActivity {
    ...
    private void onSendOrStopClicked(){
        ...
        executor.execute(()->{
            ...
            try{
                ...
                runOnUiThread(()->{adapter.add(new ChatMessage(assistantId,ChatMessage.ROLE_ASSISTANT,displayAnswer,assistantTime));scrollToEnd();setGenerating(false);setWorking(false,metric);});
    ...
    private void clearChat(){
        ...
        setGenerating(true);
        executor.execute(()->{historyStore.clear();if(engine!=null)engine.resetContext();runOnUiThread(()->{adapter.setAll(java.util.Collections.emptyList());setGenerating(false);setWorking(false,"تم مسح المحادثة والذاكرة");});});
    }
