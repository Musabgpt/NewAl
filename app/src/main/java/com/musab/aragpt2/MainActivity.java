...
public class MainActivity extends AppCompatActivity {
    ...
    private String queryDisplayName(Uri uri){
        try(android.database.Cursor c=getContentResolver().query(uri,null,null,null,null)){
            if(c!=null&&c.moveToFirst()){
                ...
                if(idx>=0){String name=c.getString(idx);if(!TextUtils.isEmpty(name))return name;}
            }
        }catch(Exception ignored){}
        return "النموذج.gguf";
    }
    ...
    private String formatMetrics(GenerationResult r){
        ...
        String first=r.firstTokenMs>=0?formatMs(r.firstTokenMs):"—";
        String total=r.totalMs>0?formatMs(r.totalMs):"—";
        return String.format(java.util.Locale.US,
                "جاهز • %d tok • %s أول token • %s إجمالي • %.1f tok/s",
                r.generatedTokens,first,total,r.tokensPerSecond);
    }
    ...
    private void clearChat(){
        ...
        executor.execute(()->{
            ...
            runOnUiThread(()->{
                ...
                setGenerating(false);
                setWorking(false,"تم مسح المحادثة والذاكرة");
            });
        });
    }
    ...
    private void setGenerating(boolean value){
        ...
        sendButton.setEnabled(!value&&engine!=null&& !inputBox.getText().toString().trim().isEmpty());
    ...
    private void setWorking(boolean working,String message){
        ...
        if(!generating)sendButton.setEnabled(!working&&engine!=null&&!inputBox.getText().toString().trim().isEmpty());
    }
