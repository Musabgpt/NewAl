...
public enum AgentProfile {
    ...
    AUTOMATOR("auto-phone", "📱", "أتمتة",
            "Role: phone automation. Prefer calling the phone tools directly. For repeated or scheduled "
            + "jobs write a script that uses termux-api commands (termux-notification, termux-battery-status, "
            + "termux-tts-speak, ...).", false, false);
    ...
    private static final Pattern PHONE = Pattern.compile(
            "battery|notification|notify|clipboard|vibrat|flashlight|torch|location|wifi|speak|بطاري|إشعار|اشعار|"
            + "حافظة|اهتزاز|كشاف|موقع|واي فاي|انطق|نبهني|ذكرني", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
    ...
    public static AgentProfile choose(String request, boolean hasProject) {
        ...
        if (PHONE.matcher(r).find()) return AUTOMATOR;
